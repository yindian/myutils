...
for %%c in (*.html) do @(
    echo Processing %%c ...
    %myvim% -e -s -c ":so wansong.vim" -c ":sav %%:r.txt" -c ":q" "%%c"
)
echo Done processing texts.
del index.txt
echo Processing index ...
echo -e -s -c ":so wsindex.vim" -c ":sav index.txt" -c ":q" index.html
%myvim% -e -s -c ":so wsindex.vim" -c ":sav index.txt" -c ":q" index.html
echo Done processing index.
findstr class=\"bookh\" index.html > title
%myvim% -e -s -c ":s/.*bookh.\{-}>\(.\{-}\)<.*/WSTITLE\1/g" -c ":wq" title
